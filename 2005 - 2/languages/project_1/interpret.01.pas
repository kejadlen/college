...
program interpret;
  ...
  procedure load_token;
    function is_whitespace: Boolean;
      begin
        is_whitespace := current_char in [' ', chr(9), chr(10), chr(12), chr(13)]
      end;
  ...
  function parse_statement(side_effect: Boolean): Boolean; 
    ...
    function parse_expression(var value: Integer): Boolean;
      {function load_token_and_parse_operand(var value: Integer): Boolean;
        begin
          load_token;
          load_token_and_parse_operand := parse_operand(value);
        end;}
      ...
      function parse_if(side_effect: Boolean): Boolean;
        ...
        if(parse_expression(value)) then
          if(current_token = 'then') then
            begin
            load_token;
            if(parse_statement(value > 0)) then
        ...
        if(parse_expression(value)) then
          while(current_token = 'do') do
            ...
            if(value > 0) then
              ...
              {writeln('1', current_line, current_token);}
              current_node := expr_node;
              current_location := expr_location;
              load_token;
              {writeln('2', current_line, current_token);}
              parse_expression(value); { Already checked syntax above. }
              {writeln('3', current_line, current_token);}
              end
            else
              begin
              {writeln('done?');}
    ...
    begin { parse_statement }
      ...
      {if(not error and not end_of_file) then
        begin
        parse_statement := false;
...
begin
  ...
  while(not end_of_file) do
    if(not parse_statement(true) and not end_of_file) then
      begin
      writeln('Syntax error.');
