...
// This actually passes data to the thread
struct httpData {
	SOCKET socket;
	string file;
};

void BuildCommon(SOCKADDR_IN *);
...
//	}
	...
	// Num_threads taken from command line
	...
	// All the filenames
	string fileNames[21];
	fileNames[0] = "/cgi-bin/Retrieve_Doc?ftp_site=ftp.rfc-editor.org&sub_dir=internet-drafts&file=all_id.txt";
	fileNames[1] = "/cgi-bin/Retrieve_Doc?ftp_site=ftp.freebsd.org&sub_dir=pub/FreeBSD/updates&file=README.TXT";
	fileNames[2] = "/cgi-bin/Retrieve_Doc?ftp_site=ftp.microsoft.com&sub_dir=softlib&file=readme.txt";
	fileNames[3] = "/cgi-bin/Retrieve_Doc?ftp_site=ftp.purdue.edu&sub_dir=pub/gtap/book&file=readme.txt";

	httpData *passToFunction[100];
	...
	for (int i=0; i<num_threads; i++) 
	{
		...
		if (Socket == INVALID_SOCKET)
			...
			if (nRet == SOCKET_ERROR)
			{
				...
				passToFunction[i]->file = fileNames[i%4];
	...
	while(num_threads) {
		dwEvent = WaitForMultipleObjects(num_threads, hThread, FALSE, INFINITE);
		for(int i=dwEvent+1; i<num_threads; i++)
			hThread[i-1] = hThread[i];
		num_threads--;
	}

	printf("Closing... total bytes received: %f\n", totalBytes/1024.0);
...
void BuildCommon(SOCKADDR_IN *psaServer)
{
	//
