...
void CALLBACK RecvData(DWORD, DWORD, LPWSAOVERLAPPED, DWORD);
int		httpServer(int);
...
void CALLBACK SendData(DWORD, DWORD, LPWSAOVERLAPPED, DWORD);
long	sendString(int);
...
struct overlapInfo {
	...
	struct overlapInfo * next;
};

struct info * Socks[MAXSOCKS];		// array of info structure pointers
WSAEVENT acceptEvent;				// event for getting connection
//WSAOVERLAPPED Overlapped[MAXSOCKS];	// array of overlapped structures
//vector<WSAOVERLAPPED *> Overlapped;	// array of overlapped structures
struct overlapInfo * overlapHead;
struct overlapInfo * overlapTail;
...
void main(int argc, char *argv[]) {
	...
	while (1) {
		...
		if((SigEvent = WSAWaitForMultipleEvents(1, &acceptEvent, FALSE, WSA_INFINITE, TRUE)) == WSA_WAIT_FAILED)
			printf("Wait for event failed: %d\n", WSAGetLastError());

		//if(!WSAResetEvent(acceptEvent)) {
			//cout<<"Can't reset event: "<<WSAGetLastError()<<endl;
			//CloseConnection(SigEvent);
		//}

		//if(SigEvent == 0) {
			if(WSAEnumNetworkEvents(Socks[0]->sock, acceptEvent, &NetEvents) == SOCKET_ERROR)
...
void MasterSocket() {
	...
	if (WSAEventSelect(temp_sock, acceptEvent, FD_ACCEPT) == SOCKET_ERROR)
...
int AcceptConnection(int EventNum)
{
	...
	//if ((Events[NumSocks] = WSACreateEvent()) == WSA_INVALID_EVENT) {
		...
		//return -1;
	//}
	//Overlapped[NumSocks].hEvent = (HANDLE)NumSocks;
	//Socks[NumSocks]->Overlapped = new WSAOVERLAPPED;
	//Socks[NumSocks]->Overlapped->hEvent = (HANDLE)NumSocks;
	//Overlapped.push_back(new WSAOVERLAPPED);
	//Overlapped[NumSocks-1]->hEvent = (HANDLE)NumSocks;
	if(NumSocks != 1) {
		overlapTail = overlapTail->next = new overlapInfo;
	}
	else {
		overlapTail = new overlapInfo;
		overlapHead = overlapTail;
	}
	overlapTail->overlap = new WSAOVERLAPPED;
	overlapTail->overlap->hEvent = (HANDLE)NumSocks;
	overlapTail->next = NULL;
	Socks[NumSocks]->overlap = overlapTail;
	...
	//rc = WSARecv(Socks[tempSock]->sock, &Socks[tempSock]->buf, 1, &num, &recv_flags, Overlapped[tempSock], (LPWSAOVERLAPPED_COMPLETION_ROUTINE) RecvData);
	rc = WSARecv(Socks[tempSock]->sock, &Socks[tempSock]->buf, 1, &num, &recv_flags, overlapTail->overlap, (LPWSAOVERLAPPED_COMPLETION_ROUTINE) RecvData);
...
void CALLBACK RecvData(DWORD dwError, DWORD num, LPWSAOVERLAPPED lpOverlapped, DWORD dwFlags)
{
	int		rc;
	DWORD	flags = 0;
	int		EventNum = int(lpOverlapped->hEvent);
	...
	if(data->request.find("\r\n\r\n") != string::npos) {
		...
		rc = WSASend(Socks[EventNum]->sock, &Socks[EventNum]->buf, 1, &num, 0, lpOverlapped, (LPWSAOVERLAPPED_COMPLETION_ROUTINE) SendData);
		if(rc == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) {
			printf("Send error: %d\n", WSAGetLastError());
			CloseConnection(EventNum);
			return;
	...
	rc = WSARecv(Socks[EventNum]->sock, &Socks[EventNum]->buf, 1, &num, &flags, lpOverlapped, (LPWSAOVERLAPPED_COMPLETION_ROUTINE) RecvData);
	if(rc == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) {
		printf("Read error: %d\n", WSAGetLastError());
		CloseConnection(EventNum);
		return;
...
void CALLBACK SendData(DWORD dwError, DWORD num, LPWSAOVERLAPPED lpOverlapped, DWORD dwFlags)
{
	int		rc;		// receive code
...
long sendString(int EventNum) {
	...
	rc = WSASend(Socks[EventNum]->sock, &Socks[EventNum]->buf, 1, &num, 0, Socks[EventNum]->overlap->overlap, (LPWSAOVERLAPPED_COMPLETION_ROUTINE) SendData);
	if(rc == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) {
...
int CloseConnection(int EventNum)
{
	...
	//cout<<EventNum<<" "<<NumSocks<<endl;
	//WSACloseEvent(Events[EventNum]);
	closesocket(Socks[EventNum]->sock);
	free(Socks[EventNum]);

	struct overlapInfo *temp = overlapHead, *hold = overlapHead;

	for(i=0; i+1<EventNum; i++) {
		hold = temp;
		temp = temp->next;
	}

	if(EventNum == 1) {
		overlapHead = temp->next;
		delete temp;
	}
	else {
		if(!temp->next)
	...
	for(temp = overlapHead; temp; temp = temp->next) {
		i = int(temp->overlap->hEvent);
		if(i >= EventNum)
			temp->overlap->hEvent = HANDLE(i - 1);
	}

	// Delete this event and information pointer from arrays.
	for (i = EventNum + 1; i < NumSocks; ++i) {
		//Events[i-1] = Events[i];
		//Socks[i]->Overlapped->hEvent -= 1;
