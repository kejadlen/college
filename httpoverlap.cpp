...
void main(int argc, char *argv[]) {
	...
	while (1) {
		...
		if(!WSAResetEvent(Events[SigEvent])) {
			...
			Socks[SigEvent]->state = ST_CLOSING;
		...
		if(SigEvent == 0) {
			...
			if(BITSET(NetEvents.lNetworkEvents,FD_ACCEPT)) {
				closedConnection = AcceptConnection(SigEvent);
			}
		}

		else if(!closedConnection && Socks[SigEvent]->state == ST_READING)
			closedConnection = RecvData(SigEvent);

		else if(!closedConnection && Socks[SigEvent]->state == ST_WRITING)
			SendData(SigEvent);
	}
}
...
int AcceptConnection(int EventNum)
{
	...
	rc = WSARecv(Socks[NumSocks]->sock, &Socks[NumSocks]->buf, 1, &num, &recv_flags, &Overlapped[NumSocks], NULL);
	if(rc == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) {
		printf("Read error: %d\n", WSAGetLastError());
		Socks[NumSocks]->state = ST_CLOSING;
		CloseConnection(NumSocks);
		++NumSocks;
		return -1;
	}
...
int RecvData(int EventNum)
{
	...
	rc = WSARecv(Socks[EventNum]->sock, &Socks[EventNum]->buf, 1, &num, &flags, &Overlapped[NumSocks], NULL);
	if(rc == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) {
		printf("Read error: %d\n", WSAGetLastError());
		Socks[EventNum]->state = ST_CLOSING;
		CloseConnection(EventNum);
...
int CloseConnection(int EventNum)
{
	...
	if (Socks[EventNum]->state == ST_WRITING) {
		Socks[EventNum]->state = ST_CLOSING;
		sendString(EventNum);
	}
